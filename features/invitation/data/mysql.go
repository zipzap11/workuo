...
func (ir *invitationRepository) GetInvitationByJobID(jobId int) ([]invitation.InvitationCore, error) {
	var invitations []Invitation
	err := ir.DB.Debug().Where("invitations.job_id = ?", jobId).Joins("User").Find(&invitations).Error
	if err != nil {
		return nil, err
	}
